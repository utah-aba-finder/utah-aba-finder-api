class AddMissingCategoryFields < ActiveRecord::Migration[7.1]
  def up
    ...
    if autism_eval
      autism_eval.category_fields.create!(
        ...
        required: true, 
        options: { 'choices' => ['ADOS-2', 'ADI-R', 'CARS-2', 'GARS-3', 'M-CHAT', 'Comprehensive Assessment'] }, 
        display_order: 1
      )
      autism_eval.category_fields.create!(
        name: 'Credentials', 
        field_type: 'select', 
        required: true, 
        options: { 'choices' => ['Licensed Psychologist', 'Developmental Pediatrician', 'Child Psychiatrist', 'Licensed Clinical Social Worker', 'Board Certified Behavior Analyst'] }, 
        display_order: 2
      )
      autism_eval.category_fields.create!(
      ...
      autism_eval.category_fields.create!(
        ...
        field_type: 'select', 
        required: false, 
        options: { 'choices' => ['1-2 weeks', '2-4 weeks', '4-6 weeks', '6+ weeks'] }, 
        display_order: 5
      )
    ...
    if speech
      speech.category_fields.create!(
        ...
        field_type: 'multi_select', 
        required: true, 
        options: { 'choices' => ['Articulation', 'Language Development', 'Fluency/Stuttering', 'Voice Disorders', 'Swallowing Disorders', 'Social Communication'] }, 
        display_order: 1
      )
      speech.category_fields.create!(
        name: 'Credentials', 
        field_type: 'select', 
        required: true, 
        options: { 'choices' => ['Speech-Language Pathologist (SLP)', 'Licensed SLP', 'ASHA Certified', 'Clinical Fellow', 'Specialist Certification'] }, 
        display_order: 2
      )
      speech.category_fields.create!(
        name: 'Age Range Served', 
      ...
      speech.category_fields.create!(
        ...
        required: false, 
        options: {}, 
        help_text: 'Available for online sessions'
      )
  ...
  def down
    ...
    CategoryField.where(name: [
      'Evaluation Types', 'Credentials', 'Age Range Served', 'Insurance Accepted', 'Report Turnaround',
      'Specialties', 'Teletherapy'
    ]).destroy_all
  end
end 